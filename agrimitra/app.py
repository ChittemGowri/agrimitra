...
with st.sidebar:
    ...
    st.markdown("""
    **Orchestrator** (NVIDIA Llama 3.3 70B + tool calling)
    plans and routes to:
...
with col_chat:
    ...
    if submit:
        if not NVIDIA_API_KEY:
            st.error("Please set NVIDIA_API_KEY in your .env file before running queries.")
...
st.markdown("---")
st.caption(
    "AgriMitra combines a Diagnosis Agent (NVIDIA Llama 3.2 Vision), an Advisory Agent "
    "(RAG over agricultural extension guides + Llama 3.3 70B), and a Market Agent "
    "(live weather + mandi prices), coordinated by a planning Orchestrator using "
