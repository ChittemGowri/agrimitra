...
local agricultural knowledge base (RAG), then asks NVIDIA NIM (via OpenAI-compatible API) to synthesize
...
def run_advisory(query: str, language: str, farmer_context: str, trace: AgentTrace) -> dict:
    ...
    trace.log(AGENT_NAME, "tool_call", f"Synthesizing advisory response in {language} via NVIDIA NIM")
